...
//print전용 GPU 커널
static __global__ void print_one() {
    printf("], ");
}
//print전용 GPU 커널
static __global__ void print_two() {
    printf("\n[");
}

void quantize_row_q8_1_cuda_debug(const float * x, void * vy, const int64_t kx, const int64_t ky, const int64_t kx_padded, cudaStream_t stream) {
    const int64_t block_num_x = (kx_padded + CUDA_QUANTIZE_BLOCK_SIZE - 1) / CUDA_QUANTIZE_BLOCK_SIZE;
    const dim3 num_blocks(block_num_x, ky, 1);
    const dim3 block_size(CUDA_QUANTIZE_BLOCK_SIZE, 1, 1);
    print_two<<<1, 1, 0, stream>>>();
    cudaDeviceSynchronize();
    quantize_q8_1<<<num_blocks, block_size, 0, stream>>>(x, vy, kx, kx_padded);
    print_one<<<1, 1, 0, stream>>>();
    cudaDeviceSynchronize();
}
