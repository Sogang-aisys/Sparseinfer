...
static void ggml_cuda_op_mul_mat_vec_q1(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    int scale_factor;
    memcpy(&scale_factor, dst->op_params, sizeof(int));
    mul_mat_vec_q1<<<block_num, block_dims, 0, stream>>>((int32_t *)src0->data, (int32_t *)src1->data, (int *)dst->data, dst->ne[0], src0->ne[0], scale_factor);
}
